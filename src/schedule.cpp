...
parse(char** arg)
{
	...
	for(; arg[0] != NULL; arg++)
	{
		...
		{
			arg++;
			args.taskgraph = string(arg[0] != NULL ? arg[0] : typeid(Taskgraph).name());
			continue;
		}
		...
		{
			arg++;
			args.platform = string(arg[0] != NULL ? arg[0] : typeid(Platform).name());
			continue;
		}
...
pelib_process(const std::map<string, pelib::Record*> &records, size_t argc, char** argv)
{
	...
	if(!args.showDescription)
	{
		...
		// Read input data
		if(records.find(args.taskgraph) != records.end() && records.find(args.platform) != records.end())
		{
			map<string, Record*>::const_iterator search_tg = records.find(args.taskgraph);
			if(search_tg == records.end())
			{
				throw PelibException("Missing taskgraph description. Aborting");
			}	
			Taskgraph tg((Taskgraph*)search_tg->second);
			map<string, Record*>::const_iterator search_pt = records.find(args.platform);
			if(search_pt == records.end())
			{
				throw PelibException("Missing platform description. Aborting");
			}
			Platform pt((Platform*)search_pt->second);
			...
			output.insert(pair<string, Record*>(string(args.lib.name), sched));
			output.insert(pair<string, Record*>(args.taskgraph, ptg));
			output.insert(pair<string, Record*>(args.platform, ppt));
			// TODO: delete taskgraphs and platform
