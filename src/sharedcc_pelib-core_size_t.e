...
int
pelib_size_t_copy(size_t s1, size_t * s2)
{
  *s2 = s1;
...
char*
pelib_size_t_string_detail(size_t a, int level)
{
 return pelib_size_t_string(a);
}

size_t
pelib_size_t_fread(size_t* buffer, FILE* stream)
{
  size_t total;
  size_t num;
  int read, has_more;
...
int
pelib_stackelem_size_t_t_init(stackelem_size_t_t *elem)
  {
...
int
pelib_stackelem_size_t_write(stackelem_size_t_t *elem, size_t buffer)
  {
...
# 43 "../include/pelib/stack.c"
    ...
    return pelib_size_t_copy(buffer, &elem->buffer);
  }

int
pelib_stackelem_size_t_read(stackelem_size_t_t *elem, size_t* buffer)
  {
...
# 52 "../include/pelib/stack.c"
    ...
    return pelib_size_t_copy(elem->buffer, buffer);
  }

int
pelib_stackelem_size_t_check(stackelem_size_t_t *elem)
  {
...
stack_size_t_t *
pelib_stack_size_t_t_alloc_collection(size_t aux)
  {
    stack_size_t_t *res;

    res = malloc(sizeof(stack_size_t_t));
...
int
pelib_stack_size_t_t_init(stack_size_t_t *s)
  {
...
int
pelib_stack_size_t_t_free(stack_size_t_t *s)
  {
...
int
pelib_stack_size_t_check(stack_size_t_t *s)
  {
...
int
pelib_stack_size_t_isempty(stack_size_t_t *s)
  {
    return s->top == 
...
int
pelib_stack_size_t_push(stack_size_t_t *s, size_t buffer)
  {
    stackelem_size_t_t * elem;

    elem = malloc(sizeof(stackelem_size_t_t));
...
# 125 "../include/pelib/stack.c"
    ...
    pelib_stackelem_size_t_t_init(elem);
    pelib_stackelem_size_t_write(elem, buffer);

    pelib_stack_size_t_push_elem(s, elem);

    return 1;
  }

int
pelib_stack_size_t_pop(stack_size_t_t *s, size_t* buffer)
  {
    stackelem_size_t_t * elem;

    if (!pelib_stack_size_t_peek(s, buffer))
      {
        return 0;
      }

    pelib_stack_size_t_pop_elem(s, &elem);
    free(elem);
...
int
pelib_stack_size_t_peek(stack_size_t_t *s, size_t* buffer)
  {
    if (s->top == 
...
# 155 "../include/pelib/stack.c"
    ...
    pelib_stackelem_size_t_read(s->top, buffer);

    return 1;
  }

int
pelib_stack_size_t_push_safe(stack_size_t_t *s, size_t buffer)
  {
    pthread_mutex_lock(&s->lock);
    pelib_stack_size_t_push(s, buffer);
...
int
pelib_stack_size_t_pop_safe(stack_size_t_t *s, size_t* buffer)
  {
    pthread_mutex_lock(&s->lock);
    pelib_stack_size_t_pop(s, buffer);
    pthread_mutex_unlock(&s->lock);
...
int
pelib_stack_size_t_push_elem(stack_size_t_t *s, stackelem_size_t_t *elem)
  {
...
int
pelib_stack_size_t_pop_elem(stack_size_t_t *s, stackelem_size_t_t **elem)
  {
...
int
pelib_stack_size_t_push_elem_safe(stack_size_t_t *s, stackelem_size_t_t *elem)
  {
...
int
pelib_stack_size_t_pop_elem_safe(stack_size_t_t *s, stackelem_size_t_t **elem)
  {
    pthread_mutex_lock(&s->lock);
    pelib_stack_size_t_pop_elem(s, elem);
    pthread_mutex_unlock(&s->lock);
...
int
pelib_stack_size_t_push_safe_managed(stack_size_t_t *s, stack_size_t_t *pool, size_t buffer)
  {
    stackelem_size_t_t *elem = 
...
# 244 "../include/pelib/stack.c"
    ...
    if (pelib_stack_size_t_pop_elem(pool, &elem) != 1)
      {
        elem = malloc(sizeof(stackelem_size_t_t));
        pelib_stackelem_size_t_t_init(elem);
      }
    ...
    pelib_stack_size_t_push_elem_safe(s, elem);

    return 1;
  }

int
pelib_stack_size_t_pop_safe_managed(stack_size_t_t *s, stack_size_t_t *pool, size_t* buffer)
  {
    stackelem_size_t_t *elem = 
...
# 262 "../include/pelib/stack.c"
    ...
    if (pelib_stack_size_t_pop_elem_safe(s, &elem) != 1)
      {
        return 0;
      }
    pelib_stackelem_size_t_read(elem, buffer);

    pelib_stack_size_t_push_elem(pool, elem);
...
static size_t continuous_read_length_size_t(cfifo_size_t_t *fifo);

static size_t continuous_write_length_size_t(cfifo_size_t_t *fifo);
...
int
pelib_cfifo_size_t_t_alloc_buffer(cfifo_size_t_t* cfifo, size_t capacity)
{
 cfifo->buffer = (size_t*)malloc(sizeof(size_t) * capacity);
...
{
 cfifo_size_t_t *fifo;
 fifo = (cfifo_size_t_t*)malloc(sizeof(cfifo_size_t_t));

 return fifo;
}

cfifo_size_t_t*
pelib_cfifo_size_t_t_alloc_collection(size_t size)
{
 cfifo_size_t_t *fifo;
 fifo = pelib_cfifo_size_t_t_alloc_struct();
 if(fifo != 
...
# 94 "../include/pelib/fifo.c"
 ...
 {
  if(pelib_cfifo_size_t_t_alloc_buffer(fifo, size) == 0)
  {
   pelib_cfifo_size_t_t_free_struct(fifo);
   return 
...
int
pelib_cfifo_size_t_t_init(cfifo_size_t_t* fifo)
{
...
int
pelib_cfifo_size_t_t_free(cfifo_size_t_t* fifo)
{
 pelib_cfifo_size_t_t_free_buffer(fifo);
 return pelib_cfifo_size_t_t_free_struct(fifo);
}

int
pelib_cfifo_size_t_t_free_struct(cfifo_size_t_t* fifo)
{
...
int
pelib_cfifo_size_t_t_free_buffer(cfifo_size_t_t* fifo)
{
 free(fifo);
 return 1;
}
...
FILE*
pelib_cfifo_size_t_t_printf(FILE* stream, cfifo_size_t_t fifo)
{
 char* str;
 str = pelib_cfifo_size_t_t_string(fifo);
 printf("%s\n", str);
 free(str);
...
FILE*
pelib_cfifo_size_t_t_printf_detail(FILE* stream, cfifo_size_t_t fifo, int level)
{
 char* str;
 str = pelib_cfifo_size_t_t_string_detail(fifo, level);
 printf("%s\n", str);
 free(str);
...
static state_t
cfifo_size_t_state(cfifo_size_t_t *cfifo)
{
...
static int
is_in_content_size_t(cfifo_size_t_t *fifo, size_t index)
  {
    int res;
    size_t read;
    state_t state;
...
char*
pelib_cfifo_size_t_t_string(cfifo_size_t_t fifo)
  {
    return pelib_cfifo_size_t_t_string_detail(fifo, 0);
  }

char*
pelib_cfifo_size_t_t_string_detail(cfifo_size_t_t fifo, int level)
{
  char *str, *elem;
  unsigned int i;
  int status;
  ...
  {
    status = is_in_content_size_t(&fifo, i);
    if (abs(status) & 2)
    {
      sprintf(str, "%s>", str);
    }
    ...
    if(status > 0)
    {
      elem = pelib_size_t_string_detail(fifo.buffer[i], level);
...
int
pelib_cfifo_size_t_push(cfifo_size_t_t* fifo, size_t elem)
  {
    if (fifo->capacity > 0 &&
 cfifo_size_t_state(fifo) != FULL)
      {
        memcpy((void*)&(fifo->buffer[fifo->write]), &elem, sizeof(size_t));
...
size_t
pelib_cfifo_size_t_fill(cfifo_size_t_t* fifo, size_t num)
{
 size_t length = pelib_cfifo_size_t_capacity(fifo) - pelib_cfifo_size_t_length(fifo);
...
size_t*
pelib_cfifo_size_t_peekaddr(cfifo_size_t_t* fifo, size_t offset, size_t *num, size_t **addr)
  {
    if ((fifo->capacity > 0 && offset < pelib_cfifo_size_t_length(fifo) && cfifo_size_t_state(fifo) != EMPTY))
      {
 size_t avail = continuous_read_length_size_t(fifo);
 if(num != 
...
# 349 "../include/pelib/fifo.c"
               )
 {
  ...
  {
   *addr = (size_t*)&(fifo->buffer[(fifo->read + offset + avail) % fifo->capacity]);
  }
  else
  {
   *addr = 
...
# 369 "../include/pelib/fifo.c"
        return (size_t*)&(fifo->buffer[(fifo->read + offset) % fifo->capacity]);
      }
    else
      {
 if(num != 
...
size_t*
pelib_cfifo_size_t_writeaddr(cfifo_size_t_t* fifo, size_t *num, size_t **addr)
  {
    if ((fifo->capacity - pelib_cfifo_size_t_length(fifo) > 0) || 1)
      {
 size_t avail = continuous_write_length_size_t(fifo);
 if(num != 
...
# 391 "../include/pelib/fifo.c"
               )
 {
  ...
  {
   *addr = (size_t*)&(fifo->buffer[(fifo->write + avail) % fifo->capacity]);
  }
  else
  {
   *addr = 
...
# 399 "../include/pelib/fifo.c"
        ...
        return (size_t*)&(fifo->buffer[fifo->write]);
      }
    else
      {
        return 
...
size_t
pelib_cfifo_size_t_pop(cfifo_size_t_t* fifo)
  {
    size_t *ptr, res;

    ptr = pelib_cfifo_size_t_peekaddr(fifo, 0, 
...
# 417 "../include/pelib/fifo.c"
 ...
 res = *ptr;
      ...
      {
 size_t def;
 pelib_size_t_init(&def);
 return def;
      }
  }

size_t
pelib_cfifo_size_t_discard(cfifo_size_t_t *fifo, size_t num)
{
 int length = pelib_cfifo_size_t_length(fifo);
...
size_t
pelib_cfifo_size_t_peek(cfifo_size_t_t* cfifo, size_t offset)
  {
    size_t *ptr, res;
    ptr = pelib_cfifo_size_t_peekaddr(cfifo, offset, 
...
int
pelib_cfifo_size_t_is_full(cfifo_size_t_t* cfifo)
  {
    return cfifo_size_t_state(cfifo) == FULL;
  }

int
pelib_cfifo_size_t_is_empty(cfifo_size_t_t* cfifo)
  {
    return cfifo_size_t_state(cfifo) == EMPTY;
  }

size_t
pelib_cfifo_size_t_pushmem(cfifo_size_t_t *fifo, size_t* mem, size_t num)
{
 size_t left, length, pushed;
 switch(cfifo_size_t_state(fifo))
 {
  case EMPTY:
  case NORMAL:
   ...
   memcpy((void*)&fifo->buffer[fifo->write], mem, length * sizeof(size_t));
   ...
   if(left < num)
   {
    ...
    pushed += pelib_cfifo_size_t_pushmem(fifo, mem + length, num - length);
   }
...
size_t
pelib_cfifo_size_t_popmem(cfifo_size_t_t *fifo, size_t* mem, size_t num)
{
 size_t i;
 ...
 switch(cfifo_size_t_state(fifo))
 {
  ...
  case REVERSE:
   ...
   length = left < num ? left : num;
                        ...
                        memcpy(mem, (void*)&(fifo->buffer[fifo->read]), length * sizeof(size_t));

                        intlength = (length * sizeof(size_t)) / sizeof(int) ;
   ...
   if(left < num)
   {
    ...
    popped += pelib_cfifo_size_t_popmem(fifo, mem + length, num - length);
   }
...
size_t
pelib_cfifo_size_t_peekmem(cfifo_size_t_t* fifo, size_t* mem, size_t num, size_t offset)
{
 size_t left, length;
 cfifo_size_t_t copy = *fifo;
 copy.read = (copy.read + offset) % copy.capacity;

 switch(cfifo_size_t_state(&copy))
 {
  case FULL:
  case REVERSE:
   ...
   memcpy(mem, (void*)&(copy.buffer[copy.read]), length * sizeof(size_t));


   if((left) < num && left > 0)
   {

    length += pelib_cfifo_size_t_peekmem(&copy, mem + length, num - length, length);
   }
...
static
size_t
continuous_read_length_size_t(cfifo_size_t_t* fifo)
{
 switch(cfifo_size_t_state(fifo))
 {
  case FULL:
...
static
size_t
continuous_write_length_size_t(cfifo_size_t_t* fifo)
{
 switch(cfifo_size_t_state(fifo))
 {
  case FULL:
...
size_t
pelib_cfifo_size_t_popfifo(cfifo_size_t_t* src, cfifo_size_t_t* tgt, size_t num)
{
 size_t requested = num;

 while(num > 0 && !pelib_cfifo_size_t_is_empty(src) && !pelib_cfifo_size_t_is_full(tgt))
 {
  size_t readl = continuous_read_length_size_t(src);
  volatile size_t writel = continuous_write_length_size_t(tgt);
  size_t length = readl < writel ? readl : writel;
  length = num < length ? num : length;
  memcpy((void*)&(tgt->buffer[tgt->write]), (void*)&(src->buffer[src->read]), length * sizeof(size_t));
  num -= length;
  pelib_cfifo_size_t_discard(src, length);
  pelib_cfifo_size_t_fill(tgt, length);
 }
...
size_t
pelib_cfifo_size_t_length(cfifo_size_t_t *cfifo)
{
 switch(cfifo_size_t_state(cfifo))
 {
  case EMPTY:
   return 0;
  break;
...
size_t
pelib_cfifo_size_t_last(cfifo_size_t_t *cfifo)
{
 size_t length = pelib_cfifo_size_t_length(cfifo);

 if(pelib_cfifo_size_t_length(cfifo) <= 0)
 {
  size_t def;
  pelib_size_t_init(&def);
  return def;
 ...
 {
  size_t* ptr = pelib_cfifo_size_t_peekaddr(cfifo, length - 1, 
...
size_t
pelib_cfifo_size_t_capacity(cfifo_size_t_t* cfifo)
{
 return cfifo->capacity;
}
# 111 "size_t.c" 2
